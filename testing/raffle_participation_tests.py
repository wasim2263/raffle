from .conftest import unexpected_response_error, make_ip
...
class TestParticipate:
    ...
    def test_get_ticket_none_left(self, client, raffle, get_ticket):
        """Can't get a ticket if none are left"""

        for n in range(raffle['total_tickets'] - 1):
            get_ticket(raffle['id'])

        # hard coding ip as ip_factory will not change the ip for the 2nd participation request
        resp1 = client.post(f"/raffles/{raffle['id']}/participate/",
                            REMOTE_ADDR='234.234.234.233')

...
class TestParticipate:
    def test_get_ticket(self, client, raffle):
        """Get a ticket to participate in a raffle_draw"""
    ...
    # def test_get_ticket_none_left(self, client, raffle_draw, get_ticket, ip_factory):
    #     """Can't get a ticket if none are left"""
    #
    #     for n in range(raffle_draw['total_tickets'] - 1):
    #         get_ticket(raffle_draw['id'])
    #
    #     resp1 = client.post(f"/raffles/{raffle_draw['id']}/participate/",
    #                         REMOTE_ADDR=ip_factory)
    #     assert resp1.status_code == 201, unexpected_response_error(resp1)
    #     resp2 = client.post(f"/raffles/{raffle_draw['id']}/participate/",
    #                         REMOTE_ADDR=ip_factory)
    #     assert resp2.status_code == 410, unexpected_response_error(resp2)
    #     assert b'Tickets to this raffle_draw are no longer available' in resp2.content
    #
    # def test_get_second_ticket_from_same_ip(self, client, raffle_draw):
    ...
    #     assert b'Your ip address has already participated in this raffle_draw' in resp2.content
    #
